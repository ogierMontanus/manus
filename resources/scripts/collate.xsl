  ...
  <xsl:template match="TEI:TEI">
    <html>
      <head>
        ...
        <script type="application/javascript" src="http://etxt.dk/skt/collate.js"><!--change path to: scripts/collate.js-->
...
-->  </xsl:template>
  ...
  <xsl:template match="@rend">
    ...
    <xsl:if test=".='movedText'">
      <xsl:attribute name="title">Transposed text</xsl:attribute>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="TEI:s">
    <xsl:param name="wit"/>
    <xsl:param name="n"/>
    <div class="movedText" title="Transposed text"><xsl:apply-templates>
            <xsl:with-param name="wit" select="$wit"/>
            <xsl:with-param name="n" select="$n"/>
  ...
  <xsl:template match="TEI:anchor">
    ...
    <xsl:if test="@type='movedText'">
      ...
      <xsl:text> </xsl:text>
      <a class="shaftIcon" id="{@xml:id}" title="Compare the transposed text - {$link/@n}"
        href="{$link/parent::TEI:linkGrp/@n}.col.xml" target="_self">˟</a>
    </xsl:if>
  </xsl:template>
  
  <xsl:template match="TEI:anchor[@type='movedTextEnd']">
    <a class="shaftIcon" title="End of transposed text"><xsl:text>]×</xsl:text></a>
  </xsl:template>
